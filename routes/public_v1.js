...
public_v1.route("/data/layanan-spesialisasi").get(async (req, res) => {
  try {
...
public_v1.route("/pasien/foto-profil/:id").get(async (req, res) => {
  try {
...
public_v1.route("/admin/foto-profil/:id").get(async (req, res) => {
  try {
    const id = req.params.id;
...
public_v1.route("/layanan-spesialisasi/foto/:id").get(async (req, res) => {
  try {
    const id = req.params.id;
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="foto_layanan_spesialisasi"`
    );
    if (response.data.foto !== null) {
      ...
      const file = path.join(
        ...
        "../public",
        "img",
        "layanan-spesialisasi-no-img.jpg"
      );
